...
SCRIPT_DIR="$BATS_CWD/test/integration/s3cmd"
...
if [ ! -f "${SCRIPT_DIR}/.s3cfg" ]; then
    >&2 echo "Template s3cmd configuration file can't be found at: ${SCRIPT_DIR}/.s3cfg"
    exit 1
fi
...
setup() {
    ...
    cat ${SCRIPT_DIR}/.s3cfg | \
        ${SED_CMD} "s/%(s3bridgehost)s/$S3_BRIDGE_HOST/g" > ${S3_CFG}
