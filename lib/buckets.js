...
function AddBucket(req, res, next) {
    ...
    var log = this.log;
    var client = this.mantaClient;
    var host = utils.parseHostname(req.headers.host);

    mod_assert.string(req.params[0], 'Bucket name is a string');

    // Deal with invalid (empty) buckets
    ...
    var bucket = req.params[0].substr(0, req.params[0].length - 1);
