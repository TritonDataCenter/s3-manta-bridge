...
function findBucketName(req, options) {
    let bucket;
    let host = parseDomainFromHostWithPort(req.headers.host);

    // typically a bucket is identified by the subdomain
    let subdomain = parseSubdomain(host);
    ...
    if (!subdomainIsBucket) {
        if (req.path().length < 1) {
            ...
            let firstDir = splitFirstDirectory(req.path()).first;
...
function sanitizeS3Filepath(path, maxLength) {
    ...
    let naughtCharsRemoved = resolved
