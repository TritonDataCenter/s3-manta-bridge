...
function findBucketName(req, options) {
    var bucket;
    var host = parseDomainFromHostWithPort(req.headers.host);

    // typically a bucket is identified by the subdomain
    var subdomain = parseSubdomain(host);
    ...
    if (!subdomainIsBucket) {
        if (req.path().length < 1) {
            ...
            var firstDir = splitFirstDirectory(req.path()).first;
...
function sanitizeS3Filepath(path, maxLength) {
    var maxLen = maxLength || 1024;
    if (path.length > maxLen) {
        throw new Error('File path is larger than the maximum length of ' + maxLen);
    }

    var trimmed = mod_lo.trim(path);
    var normalized = mod_path.normalize(trimmed);
    var resolved = mod_path.resolve('/', normalized);

    var naughtCharsRemoved = resolved
        // Replace control characters
        /* eslint-disable no-control-regex */
        .replace(/[\x00-\x1f\x80-\x9f]/g, '');
        /* eslint-enable no-control-regex */
    
    if (!mod_path.isAbsolute(naughtCharsRemoved)) {
        throw new Error('Path must be absolute. Actual path: ' + naughtCharsRemoved);
    }

    return naughtCharsRemoved;
}
 ...
 */
function interpolateEnvVars(object) {
...
module.exports = {
    ...
    findBucketName: findBucketName,
    sanitizeS3Filepath: sanitizeS3Filepath,
