...
class Utils {
    ...
    static parseDomainFromHostWithPort(hostWithPort) {
        mod_assert.string(hostWithPort,
            `Host with port must be specified as a string. Actually: ${hostWithPort}`);

        if (mod_lo.isEmpty(hostWithPort)) {
            return hostWithPort;
        }
    ...
    static splitFirstDirectory(path) {
        mod_assert.string(path,
            `Path must be specified as a string. Actually: ${path}`);
    ...
    static sanitizeS3Filepath(path, maxLength) {
        ...
        // Resolve will strip trailing spaces on directories, so we add it back
        if (mod_lo.endsWith(normalized, '/') && !mod_lo.endsWith(resolved, '/')) {
            resolved += '/';
        }
