...
module.exports = class MantaClientFactory {
    ...
    static _findMantaUrl(options) {
        ...
        if (mod_lo.isEmpty(options.manta.url)) {
            return 'https://us-east.manta.joyent.com:443';
        }
    ...
    static _isRejectUnauthorizedEnabled(options) {
        if (!mod_lo.isEmpty(options.manta.insecure)) {
            if (mod_lo.isBoolean(options.manta.insecure)) {
                return !options.manta.insecure;
            } else if (mod_lo.isNumber(options.manta.insecure)) {
                return options.manta.insecure !== 1;
            } else {
                return options.manta.insecure !== '1';
            }
        } else {
    ...
    static create(options) {
        ...
        if (!mod_lo.isEmpty(options.manta.subuser)) {
            ...
            params.sign.subuser = options.manta.subuser;
        }
