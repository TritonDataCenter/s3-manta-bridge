...
class Options {
    ...
    constructor(config) {
        ...
        /**
         * When true the path used to calculate the authorization signature does not
         * contain the bucket path in it. For example, a request made to the path /bucket/foo/bar
         * would have the bucket name /bucket/ removed from it only in the authorization header.
         * When enabled it will allow for compatibility with s3cmd 1.6.0, but break compatibility
         * with s3cmd 1.6.1+ when operation on a host that doesn't support subdomains.
         *
         * @type {boolean}
         * @default false
         */
        this.stripBucketPathFromAuth = Options.loadOption(config, 'stripBucketPathFromAuth', false, 'boolean');
         ...
         * @default 1073741824
         */
        this.maxRequestBodySize = Options.loadOption(
            config, 'maxRequestBodySize', 1073741824, 'integer');
         ...
         * @type {string}
         */
        this.secretKey = Options.loadOption(config, 'secretKey');
         ...
         * @default 900000
         */
        this.maxAllowedSkewMilliseconds = Options.loadOption(
            config, 'maxAllowedSkewMilliseconds', 900000, 'integer');
         ...
         * @type {object}
         */
        this.durabilityMappingToStorageClass =
            Options.loadOption(config, 'durabilityMappingToStorageClass',
            {
                '2': 'STANDARD',
