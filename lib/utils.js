...
function parseDomainFromHostWithPort(hostWithPort) {
    ...
    if (hostWithPort.length < 1 || hostWithPort.substring(0, 1) === ':') {
        return hostWithPort;
    }
...
module.exports = {
    parseHostname: function (hostWithPort) {
        return parseDomainFromHostWithPort(hostWithPort);
    },
    parseSubdomain: function (domain) {
        return parseSubdomain(domain);
    }
};
