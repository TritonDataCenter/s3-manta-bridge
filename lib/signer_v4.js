...
class SignerV4 {
    ...
    authenticate(req, res, next) {
        ...
        let authDetails = SignerV4.buildAuthorization(req, headers, method,
    ...
    static buildHashedPayload(req) {
        return req.headers['x-amz-content-sha256'] || EMPTY_BODY_HASH;
    }

    static buildCanonicalQueryString(queryParams) {
        ...
        for (let i = 0; i < keys.length; i++) {
            ...
            canonicalQueryString += SignerV4.uriEncode(keys[i]);
            canonicalQueryString += '=';
            canonicalQueryString += SignerV4.uriEncode(queryParams[keys[i]]);
        }

        return canonicalQueryString;
    }

    static uriEncode(urlString) {
        return urlString.replace(/[^a-zA-Z0-9\-_\.~]/g, function encodeChars(c) {
            return encodeURIComponent(c);
        });
    }

    static buildCanonicalHeaders(req, signedHeaders) {
    ...
    static buildCanonicalRequest(method, path, queryParams, canonicalHeaders, signedHeaders,
                          hashedPayload) {
        ...
        canonicalRequest += '\n';
        canonicalRequest += this.buildCanonicalQueryString(queryParams);
        canonicalRequest += '\n';
    ...
    static buildStringToSign(iso8601Date, canonicalRequestHash, date, region, service) {
        let stringToSign = '';
    ...
    static sign(secretKey, contents, encoding) {
    ...
    static buildSigningKey(parts, secretKey) {
        let dateKey = SignerV4.sign('AWS4' + secretKey, parts.date);
        let dateRegionKey = SignerV4.sign(dateKey, parts.region);
        let dateRegionServiceKey = SignerV4.sign(dateRegionKey, parts.service);
        return SignerV4.sign(dateRegionServiceKey, 'aws4_request');
    }

    static buildSignature(signingKey, stringToSign) {
        return SignerV4.sign(signingKey, stringToSign, 'hex');
    }

    static buildAuthorization(req, headers, method, path, accessKey, secretKey) {
        let parts = SignerV4.parseAuthorization(headers.authorization);
        let hashedPayload = SignerV4.buildHashedPayload(req);
        let canonicalHeaders = SignerV4.buildCanonicalHeaders(req, parts.signedHeaders);
        let canonicalRequest = SignerV4.buildCanonicalRequest(req.method, path,
            req.query, canonicalHeaders, parts.signedHeaders, hashedPayload);
        let canonicalRequestHash = SignerV4.hashAsHex(canonicalRequest);
        let stringToSign = SignerV4.buildStringToSign(req.headers['x-amz-date'],
            canonicalRequestHash, parts.date, parts.region, parts.service);
        let signingKey = SignerV4.buildSigningKey(parts, secretKey);
        let signature = SignerV4.buildSignature(signingKey, stringToSign);
