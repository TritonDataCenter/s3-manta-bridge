...
function buildCanonicalQueryString(queryString) {
    if (!queryString) {
        return '';
    }

    var params = mod_qs.parse(queryString);
    ...
    for (var i = 0; i < keys.length; i++) {
        ...
        canonicalQueryString += s3UriEncode(keys[i]);
        canonicalQueryString += '=';
        canonicalQueryString += s3UriEncode(params[keys[i]]);
    }
...
function buildCanonicalRequest(method, path, query, canonicalHeaders, signedHeaders,
    ...
    canonicalRequest += '\n';
    canonicalRequest += buildCanonicalQueryString(query);
    canonicalRequest += '\n';
