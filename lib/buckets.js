...
function BucketExists(req, res, next) {
    ...
    // If we are at the base service endpoint subdomain
    if (subdomain === null || subdomain === options.baseSubdomain) {
        res.send(200);
        return next();
    }

    var bucketDir = options.bucketPath + '/' + subdomain;
